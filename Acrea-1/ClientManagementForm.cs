...
namespace ACREA
{
    public partial class ClientManagementForm : Form
    {
        ...
        public ClientManagementForm(string actionButtonText, string name = "", string phone = "")
        {
            InitializeComponent();
            ...
            {
                this.Name = name;
                this.Phone = DataModel.GetPhoneNumberFormat(phone);
        ...
        private async void clientActionButton_Click(object sender, EventArgs e)
        {
            switch (clientActionButton.Text)
            {
                case "Создать":
                    ...
                    var id = DataModel.SetClientId();
                    await DataModel.InsertClient(id, Name, Phone);
                    break;
                case "Редактировать":
